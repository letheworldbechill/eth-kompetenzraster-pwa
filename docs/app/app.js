...
function toggleReadAloud() {
    if (!speechSynthesis) {
        showError('Vorlesen wird von diesem Browser nicht unterstützt', 'warning');
        return;
    }
...
function startReading() {
    ...
    // Text aus dem aktuellen Modul extrahieren
    const contentEl = document.getElementById('detail-content');
    const tasksEl = document.getElementById('detail-tasks');
    const titleEl = document.getElementById('detail-title');

    let textToRead = titleEl.textContent + '. ';
    textToRead += contentEl.innerText + ' ';
    textToRead += tasksEl.innerText;
    ...
    currentUtterance.onerror = () => {
        isReading = false;
        updateReadButton();
        showError('Fehler beim Vorlesen', 'warning');
    };
...
function updateReadButton() {
    ...
    if (btn) {
        btn.textContent = isReading ? 'Stoppen' : 'Vorlesen';
